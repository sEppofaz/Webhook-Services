...
- `verein_activity_stats()` zählt nur `aktion IN ('erstellt','geaendert')`. Der Massen-Upload im Vereins-Dashboard (`/verein/upload`, `/verein/confirm-upload`) loggt aber `aktion='upload'`/`'upload_confirmed'` – fließt nie in „X neu" ein. Live-Check 2026-08-06: `vk_audit` enthält seit Bestehen nur 2 Zeilen (beide `erstellt`), nie `upload`/`upload_confirmed` → „0 neu" fast immer, unabhängig davon ob tatsächlich Termine per Upload hinzukamen.
