...
- **`_meta[key].landkreis` immer in der Form `"Landkreis X"` (Stand 2026-09-20):** Das Frontend (Landkreis-Chips, Gemeinde-Schlüssel, Fallback `"Landkreis Landshut"`) erwartet das Präfix. Die Schreibweise `"Landshut"` (ohne Präfix) erzeugte einen zweiten Landkreis-Chip und – seit v1.9 – eine doppelte Gemeinde „Ergoldsbach (Landshut)". Fund: FFW Paindlkofen stand in `_meta` **und** DB (`vereine_accounts`) auf `"Landshut"`, am 2026-09-20 in beiden auf `"Landkreis Landshut"` korrigiert (Backups `/root/*.bak-2026-09-20`). **Offene Ursache:** `lookup_plz()` (`shared/kalender_core.py`) entfernt per `re.sub(r"^(Landkreis|Kreis)\s+","",…)` das Präfix und liefert bei jeder PLZ-basierten Registrierung/Import wieder `"Landshut"` → Fix steht als Todo aus (Vorsicht: „Landshut" kann auch die kreisfreie Stadt sein).
...
- **Chip-/Titel-Größen im Filter (v1.14, 2026-09-20, live im Chrome gemessen):** `.filter-title` = 15px/700 in `var(--text)` (wie `.ev-title`, vorher 12px/600 in Akzentfarbe → wurde übersehen). Alle Chips der Filter-Card sind ~32px hoch (`.chip` 32, `.rubrik-chip` 32,5); Favoriten-Chips waren 37,5px, weil das gespeicherte Icon (`f.icon`, SVG mit 18px) die Zeile streckte – gelöst per CSS `#fav-bar .chip{display:inline-flex;align-items:center;gap:5px}` + `#fav-bar .chip>svg{width/height:14px}` (das gespeicherte SVG selbst bleibt unverändert; Entfernen-Kreuz ist ein verschachteltes SVG und davon nicht betroffen). `.fav-hint-btn` hat dieselben Maße wie ein Chip (7px 14px, 13px, radius 20px). **Neue Chip-artige Elemente in der Filter-Card:** gleiche Maße verwenden, nicht mit eigenen Paddings/Icongrößen abweichen.
